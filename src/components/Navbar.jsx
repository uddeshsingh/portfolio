...
import { Home, Briefcase, Cpu, GraduationCap, FolderGit2, Mail } from 'lucide-react';
...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // 1. Show Navbar on scroll
      setIsVisible(true);

      // 2. Clear existing timeout to reset the timer
      if (scrollTimeout.current) {
        clearTimeout(scrollTimeout.current);
      }

      // 3. Set new timeout to hide after 2 seconds of inactivity
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div 
            ...
            exit={{ y: 50, scale: 0.8, opacity: 0, transition: { duration: 0.5 } }}
              ...
              return (
                <div key={item.id} className="relative group">
                  <motion.button
                    ...
                    className={`p-3 rounded-full transition-colors relative flex items-center justify-center ${isActive ? 'text-neonBlue shadow-[0_0_10px_rgba(0,243,255,0.3)]' : 'text-gray-400 hover:text-white'}`}
                  >
                    {item.icon}
                  </motion.button>
